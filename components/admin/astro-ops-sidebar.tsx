...
import { ASTRO_OPS_BASE, astroOpsPath } from "@/lib/admin/astro-ops-paths";

const NAV = [
  { label: "Dashboard", href: ASTRO_OPS_BASE, match: (p: string) => p === ASTRO_OPS_BASE },
  { label: "Astrologers", href: astroOpsPath("/astrologers"), match: (p: string) => p.startsWith(astroOpsPath("/astrologers")) },
  { label: "Chat inbox", href: astroOpsPath("/inbox"), match: (p: string) => p.startsWith(astroOpsPath("/inbox")) },
  { label: "Sessions", href: astroOpsPath("/sessions"), match: (p: string) => p.startsWith(astroOpsPath("/sessions")) },
  { label: "Users", href: astroOpsPath("/users"), match: (p: string) => p.startsWith(astroOpsPath("/users")) },
  { label: "Wallet ledger", href: astroOpsPath("/wallet-ledger"), match: (p: string) => p === astroOpsPath("/wallet-ledger") },
  { label: "Wallet cashback", href: astroOpsPath("/settings"), match: (p: string) => p.startsWith(astroOpsPath("/settings")) },
] as const;
