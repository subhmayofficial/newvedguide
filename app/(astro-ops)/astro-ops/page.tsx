...
export default async function LiveConsultHubPage() {
  ...
  const statCards = [
    { icon: Users, label: "Registered users", value: userCount ?? 0, href: "/astro-ops/users", color: "text-brand", bg: "bg-brand/10" },
    { icon: MessageCircle, label: "Total sessions", value: sessionCount ?? 0, href: "/astro-ops/sessions", color: "text-indigo-600 dark:text-indigo-400", bg: "bg-indigo-500/10" },
    { icon: Zap, label: "Live now", value: openSessions ?? 0, href: "/astro-ops/sessions?status=open", color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-500/10" },
    { icon: Clock, label: "Awaiting join", value: waitingSessions ?? 0, href: "/astro-ops/sessions?status=waiting_astrologer", color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-500/10" },
  ];

  const quickLinks = [
    { href: "/astro-ops/inbox", label: "Chat inbox", desc: "Reply to sessions in one place" },
    { href: "/astro-ops/users", label: "End users", desc: "Profiles, wallet, activity" },
    { href: "/astro-ops/sessions", label: "All sessions", desc: "Full session table" },
    { href: "/astro-ops/wallet-ledger", label: "Wallet ledger", desc: "Top-ups and charges" },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <h1 className="mt-1 font-heading text-3xl font-bold tracking-tight">Dashboard</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Chat sessions, wallets, and ledger — separate from commerce admin
        </p>
      </div>
      ...
      {(waitingSessions ?? 0) > 0 && (
        <Link
          href="/astro-ops/sessions?status=waiting_astrologer"
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-border/60 bg-card shadow-sm overflow-hidden">
          <div className="flex items-center justify-between border-b border-border/50 px-5 py-4">
            ...
            <Link href="/astro-ops/sessions" className="flex items-center gap-1 text-xs font-semibold text-brand hover:underline">
            ...
            {(recentSessions ?? []).length === 0 ? (
              ...
              (recentSessions ?? []).map((s) => {
                ...
                return (
                  <Link
                    key={s.id}
                    href={`/astro-ops/inbox?sid=${encodeURIComponent(s.id)}`}
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-border/60 bg-card shadow-sm overflow-hidden">
            <div className="flex items-center justify-between border-b border-border/50 px-5 py-4">
              ...
              <Link href="/astro-ops/wallet-ledger" className="flex items-center gap-1 text-xs font-semibold text-brand hover:underline">
                ...
                (recentLedger ?? []).map((row) => {
                  ...
                  return (
                    <div key={row.id} className="flex items-center gap-3 px-5 py-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <Link href={`/astro-ops/users/${row.user_id}`} className="font-mono text-[11px] text-muted-foreground hover:underline">
